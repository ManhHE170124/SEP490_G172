...
export default function AdminTicketDetail() {
  ...
  const doAssign = async (assigneeId) => {
    try { await ticketsApi.assign(id, assigneeId); await load(); }
    catch (e) { alert(e.message); }
  };
  const doTransfer = async (assigneeId) => {
    try { await ticketsApi.transferTech(id, assigneeId); await load(); }
    catch (e) { alert(e.message); }
  };
  const doComplete = async () => {
    if (!window.confirm("Xác nhận đánh dấu Hoàn thành?")) return;
    try { await ticketsApi.complete(id); await load(); }
    catch (e) { alert(e.message); }
  };
  const doClose = async () => {
    if (!window.confirm("Xác nhận Đóng ticket?")) return;
    try { await ticketsApi.close(id); await load(); }
    catch (e) { alert(e.message); }
  };

  const handleQuickInsert = (template) => setReplyText((prev) => (prev ? `${prev}\n${template}` : template));
    ...
    <div className="tkd-page">
      <div className="ticket-header">
        ...
        <div className="right">
          ...
          {actions.canTransfer && (
            <button className="btn warning" onClick={() => setModal({ open: true, mode: "transfer", excludeUserId: data.assigneeId })}>
              Chuyển hỗ trợ
            </button>
          )}
          ...
          <div className="card">
            ...
            {(data.assigneeName || data.assigneeEmail) && (
              <>
                ...
                <div className="kv"><span className="k">Email NV</span><span className="v">{data.assigneeEmail || "-"}</span></div>
              </>
            )}
          </div>
