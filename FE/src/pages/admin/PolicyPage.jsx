 ...
 * Created: 2025
 * Version: 1.0.0
 * Purpose: Admin page for managing Policy static content
 */
...
const PolicyPage = () => {
  ...
  // Helper function to get PostTypeId for Policy
  const getPolicyPostTypeId = async () => {
    try {
      const postTypes = await postsApi.getPosttypes();
      const postTypesArray = Array.isArray(postTypes) ? postTypes : (postTypes?.data || []);
      const policyType = postTypesArray.find(pt => {
        const name = (pt.postTypeName || pt.PostTypeName || '').toLowerCase();
        return name === 'policy' || name.replace(/\s+/g, '-') === 'policy';
      });
      return policyType?.postTypeId || policyType?.PostTypeId || null;
    } catch (err) {
  ...
  useEffect(() => {
    const fetchPolicy = async () => {
      try {
        setLoading(true);
        const response = await staticContentApi.getPolicy();
        ...
        if (err.response?.status === 404) {
          // Try to get PostTypeId for Policy
          const policyPostTypeId = await getPolicyPostTypeId();
          if (policyPostTypeId) {
            setPostTypeId(policyPostTypeId);
          } else {
            showError('Chưa có PostType', 'Chưa có PostType "Policy". Vui lòng tạo PostType trước.');
          }
        } else {
          showError('Lỗi tải dữ liệu', 'Không thể tải thông tin chính sách.');
        }
        // Set empty content
        setContent('');
        contentLoadedRef.current = true;
  ...
  const handleSave = async () => {
    ...
    if (!finalPostTypeId) {
      finalPostTypeId = await getPolicyPostTypeId();
      if (!finalPostTypeId) {
        showError('Lỗi', 'Không tìm thấy PostType "Policy". Vui lòng tạo PostType trước.');
        return;
      }
    ...
    try {
      ...
      const postData = {
        title: title.trim(),
        slug: 'policy', // Fixed slug for Policy
      ...
      if (postId) {
        ...
        await staticContentApi.updatePolicy(postData);
        showSuccess('Lưu thành công', 'Chính sách đã được cập nhật.');
      } else {
        // Create new post using static content endpoint
        const createdPost = await staticContentApi.updatePolicy(postData);
        const createdData = createdPost?.data || createdPost;
        setPostId(createdData.postId || createdData.PostId || null);
        showSuccess('Lưu thành công', 'Chính sách đã được tạo mới.');
