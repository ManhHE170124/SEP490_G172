/**
 * File: UserGuidePage.jsx
 * Author: Keytietkiem Team
 * Created: 2025
 * Version: 1.0.0
 * Purpose: Admin page for managing UserGuide static content
 */
...
const UserGuidePage = () => {
  ...
  // Helper function to get PostTypeId for UserGuide
  const getUserGuidePostTypeId = async () => {
    try {
      const postTypes = await postsApi.getPosttypes();
      const postTypesArray = Array.isArray(postTypes) ? postTypes : (postTypes?.data || []);
      const userGuideType = postTypesArray.find(pt => {
        const name = (pt.postTypeName || pt.PostTypeName || '').toLowerCase();
        return name === 'user guide' || name === 'userguide' || name.replace(/\s+/g, '-') === 'user-guide';
      });
      return userGuideType?.postTypeId || userGuideType?.PostTypeId || null;
    } catch (err) {
  ...
  // Fetch UserGuide content
  useEffect(() => {
    const fetchUserGuide = async () => {
      try {
        setLoading(true);
        const response = await staticContentApi.getUserGuide();
        ...
        if (err.response?.status === 404) {
          // Try to get PostTypeId for UserGuide
          const userGuidePostTypeId = await getUserGuidePostTypeId();
          if (userGuidePostTypeId) {
            setPostTypeId(userGuidePostTypeId);
          } else {
            showError('Chưa có PostType', 'Chưa có PostType "User Guide". Vui lòng tạo PostType trước.');
          }
        } else {
          showError('Lỗi tải dữ liệu', 'Không thể tải thông tin hướng dẫn.');
        }
        // Set empty content
        setContent('');
        contentLoadedRef.current = true;
  ...
  const handleSave = async () => {
    ...
    if (!finalPostTypeId) {
      finalPostTypeId = await getUserGuidePostTypeId();
      if (!finalPostTypeId) {
        showError('Lỗi', 'Không tìm thấy PostType "User Guide". Vui lòng tạo PostType trước.');
        return;
      }
    ...
    try {
      ...
      const postData = {
        title: title.trim(),
        slug: 'user-guide', // Fixed slug for UserGuide
      ...
      if (postId) {
        ...
        await staticContentApi.updateUserGuide(postData);
        showSuccess('Lưu thành công', 'Hướng dẫn đã được cập nhật.');
      } else {
        // Create new post using static content endpoint
        const createdPost = await staticContentApi.updateUserGuide(postData);
        const createdData = createdPost?.data || createdPost;
        setPostId(createdData.postId || createdData.PostId || null);
        showSuccess('Lưu thành công', 'Hướng dẫn đã được tạo mới.');
        // Refresh to load the new post
        window.location.reload();
      }
    } catch (err) {
      console.error('Failed to save user guide:', err);
