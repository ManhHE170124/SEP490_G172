        ...
        private async Task ProcessVariants(List<ProductVariant> variants, Order order)
        {
            ...
            foreach (var variant in sharedAccountVariants)
            {
                ...
                var availableSlot = availableAccounts.Sum(x => x.MaxUsers) -
                                    availableAccounts
                                        .Select(x => x.ProductAccountCustomers.Count(pac => pac.IsActive))
                                        .Sum();
                ...
                foreach (var account in availableAccounts)
                {
                    try
                    {
                        ...
                        _logger.LogInformation(
                            "Added customer to shared account {AccountId} for order {OrderId}",
                            account.ProductAccountId, order.OrderId);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex,
                            "Failed to add customer to shared account {AccountId} for order {OrderId}",
                            account.ProductAccountId, order.OrderId);
                    }
                }
                ...
                if (addedToSharedAccount)
                {
                    orderProducts.Add(new OrderProductEmailDto
                    {
                        ...
                        Notes =
                            "Tài khoản chia sẻ - Vui lòng làm theo hướng dẫn trong email để hoàn tất việc thêm bạn vào family account."
                    });
                }
            }
