...
public class ProductKeyService : IProductKeyService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(filter.Status)) query = query.Where(pk => pk.Status == filter.Status);

        if (!string.IsNullOrWhiteSpace(filter.Type)) query = query.Where(pk => pk.Type == filter.Type);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (key == null) throw new InvalidOperationException("Product key không tồn tại");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (supplier == null) throw new InvalidOperationException("Nhà cung cấp không tồn tại");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (key.ExpiryDate.HasValue && key.ExpiryDate < DateTime.UtcNow) key.Status = nameof(ProductKeyStatus.Expired);
        key.Notes = dto.Notes;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (keys.Count == 0) throw new InvalidOperationException("Không tìm thấy product key nào");
