...
public class PostService : IPostService
{
    ...
    public async Task<IEnumerable<PostListItemDTO>> GetAllPostsAsync(CancellationToken cancellationToken = default)
    {
    ...
    public async Task<PostDTO> CreatePostAsync(CreatePostDTO createDto, Guid actorId, CancellationToken cancellationToken = default)
    {
        ...
        {
            var postType = await _postRepository.GetPostTypeByIdAsync(createDto.PostTypeId.Value, cancellationToken);
            if (postType == null)
            {
    ...
    public async Task UpdatePostAsync(Guid id, UpdatePostDTO updateDto, Guid actorId, CancellationToken cancellationToken = default)
    {
        ...
        {
            var postType = await _postRepository.GetPostTypeByIdAsync(updateDto.PostTypeId.Value, cancellationToken);
            if (postType == null)
            {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return comments.Select(c => new PostCommentListItemDTO
        {
            ...
            UserName = c.User != null ? (c.User.FullName ?? $"{c.User.FirstName} {c.User.LastName}".Trim()) : null,
            UserEmail = c.User?.Email,
            ReplyCount = await _context.PostComments.CountAsync(r => r.ParentCommentId == c.CommentId, cancellationToken)
        });
    }
